...
preprocess = T.Compose([T.ToPILImage(),
                        T.Grayscale(num_output_channels=1),  # 将图像转换为灰度图像
...
# 七个动作 向量参数化 XYZ 偏移、垂直角度偏移（弧度）和抓取角度（弧度）。
...
def plot_screen(screen):
    plt.imshow(screen.cpu().squeeze(0)[-1].numpy(), cmap='Greys',
               interpolation='none')
...
if __name__ == "__main__":
    ...
    for i_episode in range(num_episodes):
        ...
        state = get_screen()
        plot_screen(state)
        # 将使用连续屏幕的堆栈作为输入。通过这种方式，我们希望捕捉环境的动态。
